...
func (p *MultiProducer) PopulateWithIterator(ctx context.Context, iterator_uri string, iterator_sources ...string) error {
	...
	for _, child_p := range p.producers {
		...
		go func(child_p Producer) {
			...
			err := child_p.PopulateWithIterator(ctx, iterator_uri, iterator_sources...)
